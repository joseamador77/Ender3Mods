...
static uint32_t flattenWSColor(wsColor c);
...
static uint32_t OCArray[24*5];
static uint8_t stopIndex = 0;
...
//            DB_printf("COLORS\r\n");
            ...
            colorArray[0] = flattenWSColor(pink);
            colorArray[1] = flattenWSColor(orange);
...
//            colorArray[1] = 0x00F;
            ...
            uint8_t OCIndex = 0;
            for (uint8_t currLED = 0; currLED < numLEDsToUpdate; currLED++){
                for (int8_t bitIndex = 23; bitIndex >= 0 ;bitIndex--){
                    if (colorArray[currLED] & (1 << bitIndex)){ //1
                        OCArray[OCIndex] = HIGH_TIME;
                    }else{ //0
                        OCArray[OCIndex] = LOW_TIME;
                    }
                    OCIndex++;
                }  
            }
...
//            for(int i = 0;i<numLEDsToUpdate*24;i++){
//                DB_printf("OCValue %d = %d\r\n",i,OCArray[i]);
//            }
...
static void initLEDs(void){
    ...
    PR2 = LED_PERIOD;           // set period to 2500 Hz
...
void __ISR(_OUTPUT_COMPARE_2_VECTOR,IPL7SOFT) OCFxn(void){
    static uint8_t i = 0;
    IFS0CLR = _IFS0_OC2IF_MASK;     //clear OC flag
